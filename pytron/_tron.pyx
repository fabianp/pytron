...
cdef extern from "tron.h":
    ...
    cdef cppclass TRON:
        TRON(function *, double, int)
        tron(double *)

cdef double c_func(double *w, void *f_py, double *out, int nr_variable):
...
def minimize(f, grad, hess, x0, *args):
    ...
    f_py = lambda x: f(x, *args)
    cdef double *out
    c_func(<double *> w_np.data, <void *> f_py, out, nr_variable)
